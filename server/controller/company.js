...
exports.getCompaniesWithSameBenefit = async (req, res, next) => {
  try {
    ...
    if (BENEFIT_TYPES.indexOf(benefitType) > -1) {
      ...
      logger.debug(`[comapnies/benefit router:get] benefit type "${benefitType}" // number of companies sent : [${queryResult[0].length}]`);
      // return res.json(queryResult[0]);
    } else {
      logger.error(`[comapnies/benefit router:get] Error :  benefit type "${benefitType}" is not defined  in [${total_benefit_type}]`);
      return res.status(500).json({ error: `benefit type "${benefitType}" is not defined  in [${total_benefit_type}]` });
    }
  } catch (err) {
